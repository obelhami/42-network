...
/*   Updated: 2024/10/02 22:33:55 by obelhami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	herdoc_ut(int fd, t_lexer*tmp, char **env)
{
	int		ex_status;
	int		status;
	char	*content;
...
char	*join_delm(char *content, char *tmp, char *tmp2, int *i)
{
	int		k;
	char	*str;
	...
	while (content[i])
	{
		...
		else
		{
			if (content[i] == '$' && content[i + 1] == '$' && i % 2 == 0)
			{
				...
				tmp2 = join_delm(&content[i], tmp, tmp2, &i);
		}
	}
	return (tmp2);
}

int	handle_heredoc(t_lexer*tmp, char **env)
{
	int	pid;
	int	fd[2];
